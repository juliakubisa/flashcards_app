...
class Algorithm:
    ...
    def set_weights(self):
        ...
        # Select the rows with notnull date_last_review -> cards that were quizzed before
        ...
        if old_entries.any():
            ...
            self.df.loc[old_entries, 'days_since_last_review'] = (self.df.loc[old_entries, 'date_last_review']
                                                                  - date.today()).apply(lambda d: d.days)
            self.standarize_values(old_entries)

            weights = [-0.25, 0.25, 0.25, -0.25, 0.25]
            self.df.loc[old_entries, 'new_easiness'] = (weights[1] * self.df.loc[old_entries, 'last_answer_correct']
                                       + weights[2] * self.df.loc[old_entries, 'number_correct_answers_s']
                                       + weights[3] * self.df.loc[old_entries, 'answer_time_s']
                                       + weights[4] * self.df.loc[old_entries, 'days_since_last_review'])
    ...
    def select_quiz_cards(self):
        ...
        if len(new_entries_df) >= self.num_cards:
            ...
            remaining_cards = self.num_cards - len(cards_to_quiz_ids)

            # Calculate and select the rest based on probability from the old entries
